...
def is_all_caps(s):
    ...
    if not (len(s) >= 2):
        return False
    for c in s:
        # here we check the three conditions together
        # if one of them fails, that means it is not all CAP
        if not(is_letter(c) and (c == c.upper())):
            return False # Here, if one of the c reach False, the function terminates
        #Thus the end return statement won't be reached. Otherwise, none of c reach
        #the top return statement, it goes to the last one then it's True that all are CAP
    return True
...
def word_to_braille(word):
    ...
    '''
    newstr = ''
    #create a new one for the all capital situation since it has different start pattern
    newstr_CAP = CAPITAL + '\n\n' + CAPITAL + '\n\n'
    for char in word:
        if is_all_caps(word):
            newstr_CAP += char_to_braille(char) + '\n\n'
        elif is_capitalized(char): 
            newstr += CAPITAL + '\n\n' + char_to_braille(char) + '\n\n'
        elif is_digit(char): 
            newstr += NUMBER + '\n\n' + char_to_braille(char) + '\n\n'
        else: # otherwise, including unknown characters
            newstr += char_to_braille(char) + '\n\n'
    # now, if it's cap, obviously we have a completely different patter
    # so return in two situations respectively
    if is_all_caps(word):
        return newstr_CAP
    else: 
        return newstr
...
def new_filename(fname, addition):
    ...
    name = fname[:-4]
    file_type = fname[-4:]
    return name + '_' + addition + file_type
